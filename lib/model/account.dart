...
class Account {
  ...
  Account.fromJson(Map<String, dynamic> json) {
    response = json['response'] != null ? new AccountResponse.fromJson(json['response']) : null;
  }
...
''');
  ...
  Future<AccountPersist> insert(AccountPersist todo) async {
    todo.id = await db.insert(tableAccount, todo.toJson(), conflictAlgorithm: ConflictAlgorithm.replace);
    return todo;
  }
  ...
  Future<int> update(AccountPersist todo) async {
    return await db.update(tableAccount, todo.toJson(), where: '$columnId = ?', whereArgs: [todo.id]);
  }
...
class AccountResponse {
  ...
  AccountResponse({this.accessToken, this.expiresIn, this.tokenType, this.scope, this.refreshToken, this.user, this.deviceToken});
...
class User {
  ...
  User({this.profileImageUrls, this.id, this.name, this.account, this.mailAddress, this.isPremium, this.xRestrict, this.isMailAuthorized});
