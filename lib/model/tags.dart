...
class AutoWords {
  ...
  factory AutoWords.fromJson(jsonRes) {
    ...
    List<IllustTags> tags = jsonRes['tags'] is List ? [] : null;
    if (tags != null) {
      for (var item in jsonRes['tags']) {
        if (item != null) {
          tags.add(IllustTags.fromJson(item));
        }
      }
    }
...
class IllustTags {
  ...
  factory IllustTags.fromJson(jsonRes) => jsonRes == null
      ? null
      : IllustTags(
          name: jsonRes['name'],
...
class TagsPersist {
  ...
  Map<String, dynamic> toMap() {
    var map = <String, dynamic>{columnId: id, columnName: name, columnTranslatedName: translatedName};
    if (id != null) {
      map[columnId] = id;
    }
    return map;
...
''');
  ...
  Future<TagsPersist> getTodo(int id) async {
    List<Map> maps = await db.query(tableTag, columns: [columnId, columnName, columnTranslatedName], where: '$columnId = ?', whereArgs: [id]);
    if (maps.length > 0) {
  ...
  Future<List<TagsPersist>> getAllAccount() async {
    ...
    List<Map> maps = await db.query(tableTag, columns: [columnId, columnName, columnTranslatedName]);
  ...
  Future<int> update(TagsPersist todo) async {
    return await db.update(tableTag, todo.toMap(), where: '$columnId = ?', whereArgs: [todo.id]);
  }
